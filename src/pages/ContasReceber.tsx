...
const statusOpts = ["Em aberto", "Recebido", "Vencido", "Cancelado"];
...
const ContasReceber = () => {
  ...
  const [form, setForm] = useState<Conta>({
    numero: "", cliente: "", descricao: "",
    emissao: new Date().toISOString().slice(0, 10),
    vencimento: new Date().toISOString().slice(0, 10),
    valor: 0, formaRecebimento: "Boleto", status: "Em aberto",
  });
  ...
  const handleSalvar = () => {
    if (!form.cliente || !form.descricao) return;
  ...
  const filtered = items.filter((x) =>
    x.numero.includes(search) || x.cliente.toLowerCase().includes(search.toLowerCase()) ||
  ...
  return (
    <div>
      <fieldset className="fieldset-retro">
        <legend>Módulo Financeiro — Contas a Receber</legend>

        <div className="flex items-center gap-2 mb-3">
          <label className="text-[13px] font-medium">Buscar:</label>
          <input type="text" className="input-retro flex-1" placeholder="Nº, cliente ou descrição..."
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={i} className={i % 2 === 0 ? "bg-background" : "bg-secondary/30"}>
                  ...
                  <td className="p-1 px-2">{c.cliente}</td>
                  ...
                  <td className="p-1 px-2">
                    <span className={`px-2 py-0.5 text-[11px] rounded ${
                      c.status === "Recebido" ? "bg-green-200 text-green-800" :
        ...
        <div className="flex justify-between items-center mt-2 text-[12px]">
          ...
          <span className="font-bold">Total a receber: R$ {totalAberto.toFixed(2)}</span>
        </div>
      </fieldset>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="window-frame w-[600px]">
            <div className="window-titlebar">{editIndex !== null ? "Editar Conta a Receber" : "Nova Conta a Receber"}</div>
            <div className="p-4 space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div>
                  <label className="text-[12px] font-medium">Forma Recebimento</label>
                  <select className="input-retro w-full" value={form.formaRecebimento}
